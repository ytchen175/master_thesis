...
from transformers import BitsAndBytesConfig, LlamaTokenizer, AutoModelForCausalLM
...
def main():
    RESULT_PATH = "google_inference_result_org_ch_alpaca.csv"
    ...
    google_res_df = pd.read_csv('./exp_outputs/raw_inference_results/google_translate_result.csv')

    pb = tqdm(total=len(google_res_df), nrows=4, position=0, leave=True)

    for _, row in google_res_df.iterrows():
        ...
        baseline_response = row['ch_google']
